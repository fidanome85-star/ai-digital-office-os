...
| Model evaluation, memory query, cost summary | VERIFIED as built | golden path exercises all three; memory query and cost summary each had a documented placeholder limit (no embedding provider / no budget table) at the time — see ADR 0002 §5. Model evaluation's placeholder (no benchmark harness) is closed by a later fix; see the "Follow-up fix" row after Phase 7 below and ADR 0011 |
...
| **Follow-up fix (post-Phase-9 hardening):** `POST /deployments` to `environment: "production"` requires an APPROVED `DEPLOYMENT_PRODUCTION` `approval_request_id` | VERIFIED | `pnpm --filter @ai-office/control-plane-api run test` — `integration-wiring.test.ts`'s "production approval gate" suite: no `approval_request_id` → 403; an unapproved one → 403; an APPROVED one → 201; a non-production environment is unaffected. Closes `docs/blueprint/control_plane_api_v1.4_closure.md` contract rule 9 ("Production deployment and agent activation must use explicit approval state"), which agent version activation already enforced but deployment creation did not |
| **Follow-up fix:** session-management boundary explicitly documented (integrity review MEDIUM finding) | VERIFIED by construction | `packages/auth/src/index.ts` top-of-file comment + ADR 0010 — `packages/auth` was already stateless bearer-JWT verification with no session store; the finding was that this had never been written down, not that the architecture was wrong |
| **Follow-up fix:** `POST /models/evaluate` runs a real benchmark harness instead of carrying `model_registry.evaluation_score` forward | VERIFIED | `pnpm --filter @ai-office/model-router-gateway run test` (new `evaluate.test.ts`, 3/3) + `pnpm --filter @ai-office/control-plane-api run test` (`integration-wiring.test.ts`'s new case: a model pre-seeded with `evaluation_score = 12.5` scores a real `100` after a fully successful benchmark run against `local-echo`, not the carried-forward value; real `model_evaluation_metrics` rows — `success_rate`, `avg_latency_ms`, `avg_output_tokens` — are asserted directly). The score measures execution reliability, not answer correctness — see ADR 0011 for why |
